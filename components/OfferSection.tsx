...
export default function OfferSection() {
  ...
  // ── Animations ──
  const heroY = useTransform(scrollYProgress, [0, 0.15], ["25vh", "0vh"]);
  ...
  // Text Swap (Snappy)
  const soloOpacity = useTransform(scrollYProgress, [0.1, 0.15], [0, 1]);
  const notAtGrindOpacity = useTransform(scrollYProgress, [0.1, 0.15], [1, 0]);
  ...
  return (
    <section ref={containerRef} className="relative h-[700vh] bg-[#E4E2DC]">
      {/* Heading */}
      <div className="absolute top-0 w-full flex flex-col items-center pt-16 px-6 text-center z-50 pointer-events-none">
        <h1 className="font-black uppercase text-[#111] leading-[0.9] tracking-tight text-5xl md:text-7xl max-w-xl">
          YOU DON&apos;T
          <br />
          HAVE TO
          <br />
        ...
        <p className="mt-5 text-sm text-zinc-500 font-normal max-w-xs leading-relaxed">
          The Grind is not a fitness hype, but your path to physical and mental
          growth.
        </p>
      </div>
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        <motion.div
          ...
          className="grid grid-cols-5 gap-3 w-full max-w-[1300px] px-8"
        >
        ...
        <motion.div
          ...
          style={{
            ...
            y: heroY,
            width:
              "clamp(140px, calc((min(1300px, 100vw - 64px) - 48px) / 5), 260px)",
            aspectRatio: "1 / 1",
          }}
        >
          <div className="relative w-full h-full rounded-2xl overflow-hidden bg-zinc-900 shadow-2xl">
            <Image
              src={gridImages[7]}
              alt="Hero"
              fill
            ...
            <motion.div
              style={{ opacity: soloOpacity }}
              className="absolute inset-0 flex items-center justify-center p-3"
            >
              <h2 className="text-white font-black uppercase text-center leading-[0.88] text-[clamp(1.4rem,2.6em,3rem)] tracking-[-0.02em]">
                SOLO
                <br />
                MISSION?
              </h2>
            ...
            <motion.div
              style={{ opacity: notAtGrindOpacity }}
              className="absolute inset-0 flex items-center justify-start p-4"
            >
              <h2 className="text-white font-black uppercase leading-[0.88] text-[clamp(1.2rem,2.4em,2.8rem)] tracking-[-0.02em]">
                NOT AT
                <br />
                THE GRIND
              </h2>
